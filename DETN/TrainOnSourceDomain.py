...
def Train(args, model, train_dataloader, optimizer, epoch, writer):
    ...
    if args.Backbone == 'ResNet18' or args.Backbone == 'ResNet50':
        if epoch <= 20:
            args.lr = 1e-4
        elif epoch <= 40:
            args.lr = 1e-5
    ...
    Total Loss {loss:.4f} Global Cls Loss {global_cls_loss:.4f} Local Cls Loss {local_cls_loss:.4f}'''.format(acc_avg, prec_avg, recall_avg, f1_avg,
                                                                                                              loss=loss.avg, 
...
def Test(args, model, test_source_dataloader, test_target_dataloader, Best_Recall):
    ...
    acc, prec, recall = [AverageMeter() for i in range(7)], [AverageMeter() for i in range(7)], [AverageMeter() for i in range(7)]
    loss, global_cls_loss, local_cls_loss, data_time, batch_time =  AverageMeter(), AverageMeter(), AverageMeter(), AverageMeter(), AverageMeter()
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if not args.isTest:
            Train(args, model, train_dataloader, optimizer, epoch, writer)
