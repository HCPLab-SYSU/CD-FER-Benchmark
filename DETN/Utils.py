...
def GetIndexFromDataset(model, source_dataset, target_dataset):

    source_index, target_index = [[] for i in range(7)], [[] for i in range(7)]

    # Source Index
    for index in range(len(source_dataset.imgs)):
        source_index[source_dataset.labels[index]].append(index)

    model.eval()  

    # Target Index
    for index in range(len(target_dataset.imgs)):

        trans_img, landmark, label = target_dataset[index]
        trans_img, landmark = trans_img.cuda(), landmark.cuda()
        with torch.no_grad():
            feature, pred, loc_pred = model(trans_img.unsqueeze(0), landmark.unsqueeze(0))
        pred = torch.argmax(pred,dim=1).cpu().data
        target_index[pred].append(index)

    model.train()

    source_info = [len(source_dataset.imgs)]
    source_info+=[len(source_index[i]) for i in range(7)]

    target_info = [len(target_dataset.imgs)]
    target_info+=[len(target_index[i]) for i in range(7)]

    return source_index, target_index, source_info, target_info

def GetDataFromDataset(dataset, index, batch_size):

    assert batch_size%7==0, "Batch Size is wrong."

    numOfPerClass = batch_size // 7

    data, landmark, label = [], [], []
    for Class in range(7):
        for i in range(numOfPerClass):
            trans_img, trans_landmark, trans_label = dataset[np.random.randint(0, len(index[Class]))]
            
            data.append(trans_img.unsqueeze(0))
            landmark.append(trans_landmark.unsqueeze(0))
            label.append(trans_label)

    data = torch.cat(data, 0)
    landmark = torch.cat(landmark, 0)
    label = torch.tensor(label)

    return data, landmark, label