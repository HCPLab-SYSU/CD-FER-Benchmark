...
def Train(args, model, train_source_dataloader, train_target_dataloader, optimizer, epoch, writer):
    ...
    cls_loss, dis_loss, data_time, batch_time =  AverageMeter(), AverageMeter(), AverageMeter(), AverageMeter()
    ...
    for batch_index in range(num_iter):
        ...
        feature_source, pred1_source, pred2_source = model(data_source, landmark_source)
        feature_target, pred1_target, pred2_target = model(data_target, landmark_target)
        batch_time.update(time.time()-end)

        # Compute Loss
        cls_loss_ = nn.CrossEntropyLoss()(pred1_source, label_source) + nn.CrossEntropyLoss()(pred2_source, label_source)
        dis_loss_ = SWD(nn.Softmax(dim=1)(pred1_target), nn.Softmax(dim=1)(pred2_target))

        # Back Propagation

        # Step 1: min L_s(X_s, Y_s) -> G, C1, C2 
        ...
        # Step 2: min L_s(X_s, Y_s)-L_dis(X_t) -> C1, C2 
        for param in model.parameters():
        ...
        optimizer.zero_grad()
        loss_ = cls_loss_ - dis_loss_
        loss_.backward(retain_graph=True)
        optimizer.step()

        # Step 3: min L_dis(X_t) -> G
        for param in model.parameters():
            param.requires_grad = True
        ...
        Compute_Accuracy(args, pred1_source, label_source, acc, prec, recall)

        # Log loss
        cls_loss.update(float(cls_loss_.cpu().data.item()))
        dis_loss.update(float(dis_loss_.cpu().data.item()))
    ...
    writer.add_scalar('Cls_Loss', cls_loss.avg, epoch)
    ...
    Cls Loss {cls_loss:.4f} Dis Loss {dis_loss:.4f}'''.format(acc_avg, prec_avg, recall_avg, f1_avg,
                                                              cls_loss=cls_loss.avg, dis_loss=dis_loss.avg)
...
def Test(args, model, test_source_dataloader, test_target_dataloader, Best_Recall):
    ...
    for step, (input, landmark, label) in enumerate(iter_source_dataloader):
        ...
        with torch.no_grad():
            ...
            feature, pred1, pred2 = model(input, landmark)
            batch_time.update(time.time()-end)

        # Compute accuracy, precision and recall
        Compute_Accuracy(args, pred1, label, acc, prec, recall)
...
def main():
    ...
    train_dataloader = BulidDataloader(args, flag1='train', flag2='source')
