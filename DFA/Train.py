...
from Loss import Entropy, DANN, CDAN, HAFN, SAFN, DFA
from Utils import *
...
parser.add_argument('--alpha', type=float, default=1, help='weight of first part of dfa loss(default: 1)')
...
def Train(args, model, source_index, source_info, train_source_dataloader, train_target_dataloader, optimizer, epoch, writer):
    """Train."""
    ...
    loss, global_cls_loss, local_cls_loss, dfa_loss = AverageMeter(), AverageMeter(), AverageMeter(), AverageMeter()
    ...
    num_iter = max(source_info[0] // args.train_batch_size, len(train_target_dataloader))

    iter_target_dataloader = iter(train_target_dataloader)

    Lambda1 = 1.0/(2*args.train_batch_size) # 1.0/(len(train_source_dataloader.dataset)+len(train_target_dataloader.dataset))
    Lambda2 = 1.0/(2*args.train_batch_size) # 1.0/(len(train_source_dataloader.dataset)+len(train_target_dataloader.dataset)) 

    end = time.time()  
    for batch_index in range(num_iter):
        
        data_source, landmark_source, label_source = GetDataFromDataset(train_source_dataloader.dataset, source_index, args.train_batch_size)
        try:
        ...
        gist_loss_, intra_loss_, inter_loss_ = DFA(feature, source_info)

        loss_ = global_cls_loss_ + local_cls_loss_ + \
                args.alpha * gist_loss_ + Lambda1 * intra_loss_ - Lambda2 * inter_loss_
        ...
        dfa_loss.update(float((args.alpha * gist_loss_ + Lambda1 * intra_loss_ + Lambda2 * inter_loss_).cpu().data.item()))
        ...
        writer.add_scalar('DFA_Loss', float((args.alpha * gist_loss_ + Lambda1 * intra_loss_ + Lambda2 * inter_loss_).cpu().data.item()), num_iter*(epoch-1)+batch_index)
    ...
    Total Loss {loss:.4f} Global Cls Loss {global_cls_loss:.4f} Local Cls Loss {local_cls_loss:.4f} DFA Loss {dfa_loss:.4f}'''.format(acc_avg, prec_avg, recall_avg, f1_avg,
                                                                                                                                      loss=loss.avg, 
                                                                                                                                      global_cls_loss=global_cls_loss.avg, 
                                                                                                                                      local_cls_loss=local_cls_loss.avg, 
                                                                                                                                      dfa_loss=dfa_loss.avg)
...
def main():
    ...
    source_index, source_info = GetIndexFromDataset(train_source_dataloader.dataset)
    for epoch in range(1, args.epochs + 1):

        if not args.isTest:
            Train(args, model, source_index, source_info, train_source_dataloader, train_target_dataloader, optimizer, epoch, writer)  
