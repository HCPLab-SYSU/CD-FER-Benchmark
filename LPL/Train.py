...
def Train(args, model, train_dataloader, optimizer, epoch, writer):
    ...
    centers = getCenters(train_dataloader, model)

    model.train()
    torch.autograd.set_detect_anomaly(True)

    acc, prec, recall = [AverageMeter() for i in range(7)], [AverageMeter() for i in range(7)], [AverageMeter() for i in range(7)]
    loss, global_cls_loss, local_cls_loss, lp_loss, data_time, batch_time =  AverageMeter(), AverageMeter(), AverageMeter(), AverageMeter(), AverageMeter(), AverageMeter()
    ...
    for step, (input, landmark, label) in enumerate(train_dataloader):
        ...
        local_cls_loss_ = nn.CrossEntropyLoss()(loc_output, label)
        lp_loss_ = LP_Loss(feature, label, centers)
        # lp_loss_ = LP_Loss(feature, label, train_dataloader, model)

        loss_ = global_cls_loss_ + local_cls_loss_ + lp_loss_
        ...
        lp_loss.update(float(lp_loss_.cpu().data.item()))
    ...
    writer.add_scalar('LP_Loss', lp_loss.avg, epoch)
    ...
    Total Loss {loss:.4f} Global Cls Loss {global_cls_loss:.4f} Local Cls Loss {local_cls_loss:.4f} LP Loss {lp_loss:.4f}'''.format(acc_avg, prec_avg, recall_avg, f1_avg,
                                                                                                                                    loss=loss.avg, 
